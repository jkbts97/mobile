...
class StyleConfigManager {
    ...
    async loadConfigFromDataBank() {
        try {
            ...
            if (!configAttachment) {
                ...
                const mobileConfigs = globalAttachments.filter(att =>
                    att.name.startsWith('mobile_config_') &&
                ...
                console.log('[Style Config Manager] 找到带时间戳的配置文件:', mobileConfigs.map(c => c.name));
    ...
    async saveConfigToDataBank() {
        try {
            ...
            // 添加时间戳前缀确保文件名唯一，并明确指定.json扩展名
            const timestamp = Date.now();
            const safeFileName = `mobile_config_${timestamp}_${STYLE_CONFIG_FILE_NAME}`;
            console.log('[Style Config Manager] 安全文件名:', safeFileName);
            ...
            // 验证返回的URL是否是JSON格式
            const isValidJsonUrl = fileUrl && (
                fileUrl.endsWith('.json') ||
                fileUrl.includes(safeFileName.replace('.json', ''))
            );
            ...
            if (fileUrl && isValidJsonUrl) {
                ...
                setTimeout(async () => {
                    try {
                        const globalAttachments = getDataBankAttachmentsForSource('global', true);
                        const savedConfig = globalAttachments.find(att =>
                            att.name === STYLE_CONFIG_FILE_NAME ||
                            att.name.includes('mobile_config_') && att.name.endsWith('.json')
                        );
                ...
                console.error('[Style Config Manager] SillyTavern的uploadFileAttachmentToServer函数有问题，JSON文件被保存为TXT');
                return false;
            }
...
${config.messageReceivedAvatars ?
    config.messageReceivedAvatars.map((avatar, index) => {
        if (avatar.friendId && avatar.friendId.trim()) {
            console.log(`[Style Config Manager] ✅ 生成接收头像CSS: ${avatar.name || `头像${index + 1}`} (ID: ${avatar.friendId})`);
            console.log(`[Style Config Manager] 头像配置数据:`, avatar);
            // 生成两种CSS选择器以覆盖不同的页面结构
            const css1 = generateAvatarCSS(avatar, `.message-item[data-friend-id="${avatar.friendId}"] .message-avatar`);
            ...
            console.warn(`[Style Config Manager] ⚠️ 跳过无效头像配置: ${avatar.name || `头像${index + 1}`} - 缺少好友ID`);
            return '';
        }
    }).join('\n') : ''}
        `.trim();
    ...
    async getAllStyleConfigs() {
        try {
            if (sillyTavernCoreImported && getDataBankAttachmentsForSource) {
                // 从Data Bank获取配置文件列表
                const globalAttachments = getDataBankAttachmentsForSource('global', true);
                const styleConfigs = globalAttachments.filter(att =>
                    att.name.endsWith('_style_config.json')
                );

                // 确保默认配置只出现一次，并放在最前面
                const defaultConfigs = styleConfigs.filter(att => att.name === STYLE_CONFIG_FILE_NAME);
                const userConfigs = styleConfigs.filter(att => att.name !== STYLE_CONFIG_FILE_NAME);
                ...
                console.log('[Style Config Manager] 找到配置文件:', finalConfigs.map(c => c.name));
                console.log('[Style Config Manager] 清理了重复的默认配置:', defaultConfigs.length - (defaultConfigs.length > 0 ? 1 : 0));
                return finalConfigs;
            }
    ...
    async generateConfigListSection() {
        ...
        if (configs.length === 0) {
            ...
            configListHTML = configs.map(config => {
                const displayName = config.name.replace('_style_config.json', '').replace('mobile_style_config.json', '默认配置');
                ...
                return `
                    <div class="config-item" data-config-file="${config.name}">
                        ...
                        <div class="config-actions">
                            <button class="config-action-btn load-config" data-config-file="${config.name}" title="加载此配置">
                                📥 加载
                            </button>
                            ${!isDefault ? `
            ...
            <div class="style-config-app">
                ...
                <div class="style-config-content">
                    ...
                    ${this.generateConfigSection('messagesApp', '消息应用背景', config.messagesApp)}
                                ${this.generateAvatarConfigSection('messageSentAvatar', '发送消息头像背景', config.messageSentAvatar)}
                    ...
                    <div class="config-field image-upload-field">
                        ...
                        <div class="image-upload-container">
                            <div class="image-preview" data-field-id="${fieldId}">
                                ${value ? `<img src="${value}" alt="背景预览" />` : '<div class="no-image">📷 暂无图片</div>'}
                            </div>
                            <div class="image-upload-controls">
                                ...
                                ${value ? `<button type="button" class="remove-btn" data-target="${fieldId}">🗑️ 移除</button>` : ''}
                            </div>
                            <input
                                type="hidden"
                                id="${fieldId}"
    ...
    generateReceivedAvatarsSection(avatarsArray) {
        ...
        const avatarCards = avatarsArray.map((avatar, index) => {
            ...
            input.addEventListener('input', (e) => {
                ...
                if (avatarIndex !== null && property) {
                    ...
                    if (property === 'rotation' || property === 'scale') {
                        const relatedInputs = document.querySelectorAll(`[data-avatar-index="${avatarIndex}"][data-property="${property}"]`);
    ...
    updateAvatarProperty(avatarIndex, property, value) {
        ...
        // 更新预览
        if (property === 'backgroundImage' || property === 'backgroundImageUrl' ||
    ...
    async handleImageUpload(fileInput) {
        ...
        if (!validImageExtensions.includes(fileExtension)) {
            ...
            if (targetInput && imageUrl) {
                // 最后一次验证URL有效性
                const isFinalValidUrl = imageUrl.startsWith('data:') || // base64格式
