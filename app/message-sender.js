...
class MessageSender {
    ...
    async buildAndSendMessage(message) {
        ...
        const hasSpecialMessages = messageLines.some(line => {
            ...
            return voiceMessageRegex.test(trimmed) || redpackMessageRegex.test(trimmed) || stickerMessageRegex.test(trimmed);
        });
    ...
    async sendStickerMessages(messageLines) {
        ...
        if (success) {
            const summaryMessage = messageLines.length > 1 ?
                `${messageLines.length}个表情包` :
                '1个表情包';
