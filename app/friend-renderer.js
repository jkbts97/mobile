...
class FriendRenderer {
    constructor() {
        // 使用统一的正则表达式管理器
        this.contextMonitor = window['contextMonitor'] || (window['ContextMonitor'] ? new window['ContextMonitor']() : null);
    ...
    extractFriendsFromContext() {
        ...
        try {
            ...
            console.log('[Friend Renderer] 提取的联系人列表:', this.extractedFriends);
    ...
    getLastMessageForContact(chatMessages, contact) {
        ...
        if (contact.isGroup) {
            ...
            messagePatterns = [
                ...
                // 原有格式兼容（如果还有的话）
                new RegExp(`\\[我方群聊消息\\|${this.escapeRegex(contact.name)}\\|${this.escapeRegex(contact.number)}\\|[^|]+\\|([^|]+)\\|[^\\]]+\\]`, 'g'),
                new RegExp(`\\[对方群聊消息\\|${this.escapeRegex(contact.name)}\\|${this.escapeRegex(contact.number)}\\|[^|]+\\|[^|]+\\|([^\\]]+)\\]`, 'g')
            ];
        } else {
            // 私聊消息模式
            messagePatterns = [
                ...
                new RegExp(`\\[对方消息\\|${this.escapeRegex(contact.name)}\\|${this.escapeRegex(contact.number)}\\|[^|]+\\|([^\\]]+)\\]`, 'g')
            ];
        }
    ...
    renderFriendsHTML() {
        ...
        const contactsHTML = contacts.map(contact => {
            const timeStr = this.formatTime(contact.addTime);
