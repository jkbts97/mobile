...
class TaskApp {
    ...
    setupDOMObserver() {
        try {
            // 观察聊天容器的变化
            const chatContainer = document.querySelector('#chat') || document.querySelector('.mes') || document.querySelector('[data-mes]');
            if (chatContainer) {
    ...
    parseTasksFromContext() {
        try {
            ...
            const acceptedChanged = JSON.stringify(taskData.acceptedTasks.sort()) !== JSON.stringify(this.acceptedTasks.sort());
            const completedChanged = JSON.stringify(taskData.completedTasks.sort()) !== JSON.stringify(this.completedTasks.sort());
    ...
    parseTaskContent(content) {
        ...
        console.log('[Task App] 解析完成，任务数:', tasks.length, '已接受:', acceptedTasks.length, '已完成:', completedTasks.length);

        // 添加详细的调试信息
        if (tasks.length > 0) {
            console.log('[Task App] 任务详情:', tasks.map(t => `${t.id}: ${t.name}`));
        }
    ...
    renderTaskList() {
        ...
        const availableTasks = this.tasks.filter(task =>
            !this.acceptedTasks.includes(task.id) && !this.completedTasks.includes(task.id)
        );
        ...
        const taskItems = availableTasks.map(task => `
            <div class="task-item" data-task-id="${task.id}">
                <div class="task-icon">${this.getTaskIcon('available')}</div>
                <div class="task-info">
                    <div class="task-name">${task.name}</div>
                    <div class="task-id">编号: ${task.id}</div>
        ...
        return `
            <div class="task-app">
                ...
                <div class="task-tabs">
                    ...
                    <button class="task-tab ${this.currentView === 'inProgress' ? 'active' : ''}" data-view="inProgress">
                ...
                <div class="task-list">
                    <div class="task-header">
                        <div class="task-title">可接任务</div>
                        <div class="task-stats">共 ${availableTasks.length} 个任务</div>
                        <button class="refresh-tasks-btn" title="刷新任务状态">
                            🔄
                        </button>
                    </div>
                    <div class="task-grid">
    ...
    renderInProgress() {
        ...
        const taskItems = inProgressTasks.map(task => `
            <div class="task-item" data-task-id="${task.id}">
                <div class="task-icon">${this.getTaskIcon('inProgress')}</div>
                <div class="task-info">
                    <div class="task-name">${task.name}</div>
                    <div class="task-id">编号: ${task.id}</div>
                    <div class="task-description">${task.description}</div>
                    <div class="task-publisher">发布人: ${task.publisher}</div>
                    <div class="task-reward">奖励: ${task.reward}</div>
                    <div class="task-status">状态: 进行中</div>
                </div>
            </div>
        `).join('');
        ...
        return `
            <div class="task-app">
                ...
                <div class="task-list">
                    <div class="task-header">
                        <div class="task-title">进行中任务</div>
                        <div class="task-stats">共 ${inProgressTasks.length} 个任务</div>
                        <button class="refresh-tasks-btn" title="刷新任务状态">
                            🔄
                        </button>
                    </div>
    ...
    renderCompleted() {
        ...
        const taskItems = completedTasks.map(task => `
            <div class="task-item completed" data-task-id="${task.id}">
                <div class="task-icon">${this.getTaskIcon('completed')}</div>
                <div class="task-info">
                    <div class="task-name">${task.name}</div>
                    <div class="task-id">编号: ${task.id}</div>
                    <div class="task-description">${task.description}</div>
                    <div class="task-publisher">发布人: ${task.publisher}</div>
                    <div class="task-reward">奖励: ${task.reward}</div>
                    <div class="task-status">状态: 已完成</div>
                </div>
            </div>
        `).join('');
            ...
            <div class="task-app">
                ...
                <div class="task-list">
                    <div class="task-header">
                        ...
                        <div class="task-stats">共 ${completedTasks.length} 个任务</div>
                        <button class="refresh-tasks-btn" title="刷新任务状态">
                            🔄
                        </button>
                    </div>
    ...
    bindEvents() {
        ...
        console.log('[Task App] 事件绑定完成 - 标签页:', appContainer.querySelectorAll('.task-tab').length, '个, 刷新按钮:', appContainer.querySelectorAll('.refresh-tasks-btn').length, '个');
    }
