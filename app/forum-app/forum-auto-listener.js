...
class ForumAutoListener {
    ...
    async checkForChanges() {
        ...
        try {
            ...
            if (messageIncrement > 0) {
                console.log(`[Forum Auto Listener] 检测到新消息: +${messageIncrement} (${this.lastMessageCount} -> ${currentMessageCount})`);

                // 获取阈值（优先从论坛管理器，否则使用默认值）
                const threshold = (window.forumManager && window.forumManager.currentSettings)
                ...
                if (messageIncrement >= threshold) {
                    ...
                    console.log(`[Forum Auto Listener] 调试 - checkAutoGenerate存在: ${!!(window.forumManager && window.forumManager.checkAutoGenerate)}`);
                    ...
                    if (window.forumManager && window.forumManager.checkAutoGenerate) {
                        ...
                        console.warn(`[Forum Auto Listener] 无法调用自动生成 - forumManager: ${!!window.forumManager}, checkAutoGenerate: ${!!(window.forumManager && window.forumManager.checkAutoGenerate)}`);
    ...
    setupDOMObserver() {
        ...
        try {
            // 观察聊天容器的变化
            const chatContainer = document.querySelector('#chat') || document.querySelector('.chat-container') || document.querySelector('[data-testid="chat"]');
    ...
    setupUIObserver() {
        ...
        try {
            ...
            this._clickHandler = (event) => {
                ...
                const closeButton = event.target.closest('.mobile-phone-overlay, .close-button, .drawer-close, [data-action="close"]');
