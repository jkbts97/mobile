...
class ForumManager {
    ...
    showForumPanel() {
        ...
        panel.innerHTML = `
            ...
            <div style="margin-bottom: 20px;">
                ...
                <select id="forum-style-select" style="width: 100%; padding: 10px; border-radius: 5px; border: 1px solid #444; background: #eee; color: #333;">
                    <option value="贴吧老哥">贴吧老哥</option>
                    <option value="知乎精英">知乎精英</option>
                    <option value="小红书种草">小红书种草</option>
                    <option value="抖音达人">抖音达人</option>
                    <option value="B站UP主">B站UP主</option>
                    <option value="海角老司机">海角老司机</option>
                    <option value="八卦小报记者">八卦小报记者</option>
                    <option value="天涯老涯友">天涯老涯友</option>
                    <option value="校园论坛">校园论坛</option>
        ...
        // 设置当前值
        document.getElementById('forum-style-select').value = this.currentSettings.selectedStyle;
        ...
        try {
            ...
            if (!force) {
                ...
                if (increment < this.currentSettings.threshold) {
                    console.log(`[Forum Manager] [自动检查] 消息增量不足 (${increment}/${this.currentSettings.threshold})，跳过生成`);
                    return false;
                }
            } else {
    ...
    async callForumAPI(chatData) {
        try {
            ...
            // 获取风格提示词
            const stylePrompt = window.forumStyles ? window.forumStyles.getStylePrompt(this.currentSettings.selectedStyle) : '';

            console.log('📋 [论坛API] 系统提示词:');
        ...
        try {
            ...
            if (chatData.messages.length >= 1) {
                ...
                if (existingForumRegex.test(originalContent)) {
                    // 如果已存在论坛内容，在底部追加新内容
                    console.log('[Forum Manager] 检测到已存在论坛内容，在底部追加新内容');
                    ...
                    if (endIndex !== -1) {
                        // 在结束标记前插入新内容
                        const beforeEnd = originalContent.substring(0, endIndex);
                        const afterEnd = originalContent.substring(endIndex);

                        // 构建追加的内容（不包含标记）
                        const appendContent = `\n\n---\n[论坛内容更新]\n\n${forumContent}`;

                        // 合并内容
                        const newContent = beforeEnd + appendContent + '\n' + afterEnd;

                        // 更新第1楼层
                        const success = await window.mobileContextEditor.modifyMessage(0, newContent);
                        if (success) {
                            console.log('[Forum Manager] ✅ 论坛内容已追加到底部');
                            this.updateStatus('论坛内容已追加到底部', 'success');
                            return true;
                        } else {
                            throw new Error('modifyMessage返回false');
                        }
                    } else {
                        console.warn('[Forum Manager] 未找到论坛结束标记，回退到替换模式');
                        // 如果找不到结束标记，移除旧内容并添加新内容
    ...
    clearForumUICache() {
        try {
            ...
            const forumDataKeys = [
                'mobile_forum_threads',
                'mobile_forum_replies',
                'mobile_forum_cache'
            ];
    ...
    async sendReplyToAPI(replyFormat) {
        try {
            ...
            console.log('📋 [回复API] 系统提示词:');
            console.log(stylePrompt);
            console.log('\n💭 [回复API] 用户回复内容:');
            console.log(replyFormat);
            console.log('\n📝 [回复API] 完整用户消息:');
            const userMessage = `请根据以下聊天记录和用户回复，更新论坛内容：\n\n${contextInfo}\n\n用户回复：${replyFormat}`;
    ...
    async checkAutoGenerate() {
        ...
        try {
            ...
            console.log(`[Forum Manager] 检查自动生成条件: 当前消息数=${currentCount}, 已处理=${this.lastProcessedCount}, 增量=${increment}, 阈值=${this.currentSettings.threshold}`);
