...
class ShopApp {
        ...
        return `
            <div class="shop-product-list">
                <div class="shop-header">
                    <div class="shop-title">商品列表</div>
                    <div class="shop-stats">共 ${this.products.length} 件商品</div>
                </div>
    ...
    renderCart() {
        ...
        if (!this.cart.length) {
            return `
                <div class="shop-empty-state">
                    ...
                    <button class="back-to-shop-btn">返回商品列表</button>
                </div>
            `;
        }
        ...
        const cartItems = this.cart.map(item => `
            <div class="cart-item" data-product-id="${item.id}">
                ...
                <button class="remove-item-btn" data-product-id="${item.id}">删除</button>
            </div>
        `).join('');
        ...
        return `
            <div class="shop-cart">
                <div class="cart-header">
                    ...
                    <div class="cart-stats">共 ${totalItems} 件商品</div>
                </div>
                ...
                <div class="cart-footer">
                    ...
                    <div class="cart-actions">
                        <button class="back-to-shop-btn">继续购物</button>
    ...
    updateCartBadge() {
        ...
        if (cartBtn) {
            cartBtn.innerHTML = totalItems > 0 ? `🛒 (${totalItems})` : '🛒';
        }
    }
...
总计：${totalItems}件商品，¥${totalPrice.toFixed(2)}`;
    ...
    sendOrderToSillyTavern(orderSummary) {
        try {
            ...
            const bagMessages = this.cart.map(item =>
                `[背包|${item.name}|${item.type}|${item.description}|${item.quantity}]`
            ).join('');
