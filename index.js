...
function waitForAllModules() {
    return new Promise((resolve) => {
        const checkModules = () => {
            ...
            if (contextEditorReady && customAPIReady && uploadManagerReady && mesidFloorReady &&
                forumStylesReady && forumAutoListenerReady && forumUIReady && forumManagerReady && voiceMessageReady) {
...
async function initMobileContextPlugin() {
    try {
        ...
        isInitialized = true;
        console.log("[Mobile Context] v2.4 插件已加载（包含上传功能、上下文编辑器、自定义API配置、MesID楼层监听器和论坛管理器，使用SillyTavern.getContext() API集成）");
...
function initForumFeatures() {
    try {
        ...
        // 初始化论坛管理器
        window.forumManager.initialize().then(() => {
...
function initWeiboFeatures() {
    try {
        ...
        // 初始化微博管理器
        window.weiboManager.initialize().then(() => {
...
function bindSettingsControls() {
    // 启用/禁用监控
    $('#mobile_context_enabled').prop('checked', extension_settings.mobile_context.enabled).on('change', function() {
    ...
    // 监控聊天变化
    $('#mobile_context_monitor_chat').prop('checked', extension_settings.mobile_context.monitorChat).on('change', function() {
    ...
    // 监控角色变化
    $('#mobile_context_monitor_character').prop('checked', extension_settings.mobile_context.monitorCharacter).on('change', function() {
    ...
    // 监控系统事件
    $('#mobile_context_monitor_events').prop('checked', extension_settings.mobile_context.monitorEvents).on('change', function() {
    ...
    // 日志级别
    $('#mobile_context_log_level').val(extension_settings.mobile_context.logLevel).on('change', function() {
    ...
    // 最大日志条目
    $('#mobile_context_max_log_entries').val(extension_settings.mobile_context.maxLogEntries).on('change', function() {
    ...
    // 上传功能启用/禁用
    $('#mobile_upload_enabled').prop('checked', extension_settings.mobile_context.uploadEnabled).on('change', function() {
    ...
    // 上传通知启用/禁用
    $('#mobile_upload_notifications').prop('checked', extension_settings.mobile_context.showUploadNotifications).on('change', function() {
        extension_settings.mobile_context.showUploadNotifications = $(this).prop('checked');
        saveSettings();
    });

    // 上下文编辑器启用/禁用
    $('#mobile_context_editor_enabled').prop('checked', extension_settings.mobile_context.contextEditorEnabled).on('change', function() {
    ...
    // 自定义API配置启用/禁用
    $('#mobile_custom_api_enabled').prop('checked', extension_settings.mobile_context.customAPIEnabled).on('change', function() {
    ...
    // API配置按钮显示/隐藏
    $('#mobile_show_api_config_button').prop('checked', extension_settings.mobile_context.showAPIConfigButton).on('change', function() {
        ...
        const apiButton = document.getElementById('mobile-api-config-trigger');
        if (apiButton) {
            apiButton.style.display = (extension_settings.mobile_context.customAPIEnabled && extension_settings.mobile_context.showAPIConfigButton) ? 'flex' : 'none';
        }
    });
    ...
    // MesID楼层监听器设置
    $('#mobile_mesid_floor_enabled').prop('checked', extension_settings.mobile_context.mesidFloorEnabled).on('change', function() {
    ...
    $('#mobile_enable_floor_notifications').prop('checked', extension_settings.mobile_context.enableFloorNotifications).on('change', function() {
        extension_settings.mobile_context.enableFloorNotifications = $(this).prop('checked');
        saveSettings();
    });

    $('#mobile_floor_selector').val(extension_settings.mobile_context.floorSelector).on('change', function() {
    ...
    // 论坛管理器设置
    $('#mobile_forum_enabled').prop('checked', extension_settings.mobile_context.forumEnabled).on('change', function() {
    ...
    $('#mobile_forum_auto_update').prop('checked', extension_settings.mobile_context.forumAutoUpdate).on('change', function() {
    ...
    $('#mobile_forum_threshold').val(extension_settings.mobile_context.forumThreshold).on('change', function() {
    ...
    $('#mobile_forum_style').val(extension_settings.mobile_context.forumStyle).on('change', function() {
...
function registerConsoleCommands() {
    ...
    window.MobileContext.addFormat = function(name, regex, fields, description) {
        ...
        if (!name || !regex || !fields) {
            ...
            console.log("示例: MobileContext.addFormat('test', /\\[测试\\|([^|]*)\\|([^|]*)\\|([^|]*)\\|([^\\]]*)\\]/g, ['character', 'number', 'type', 'content'], '测试格式')");
            return false;
        }
    ...
    window.MobileContext.smartLoadChat = async function() {
        ...
        try {
            ...
            if (chatData) {
                console.log(`[Mobile Context] ✅ 聊天加载成功！共 ${chatData.messages.length} 条消息 (${chatData.characterName})`);
                return chatData;
            } else {
                console.error("[Mobile Context] 无法获取聊天数据");
                return null;
    ...
    window.MobileContext.quickSetupFloorMonitor = function(floorSelector = '.message') {
        ...
        window.mesidFloorMonitor.addEventListener('onFloorChanged', function(data) {
            console.log(`[楼层监听器] 🔄 楼层变化: ${data.oldCount} -> ${data.newCount} (${data.change > 0 ? '+' : ''}${data.change})`);
        });
...
console.log("");
    ...
    window.MobileContext.setForumStyle = function(styleName) {
        ...
        if (!styleName) {
            ...
            console.log("可用风格:", window.forumStyles ? window.forumStyles.getAvailableStyles() : ['贴吧老哥', '知乎精英', '小红书种草', '抖音达人', 'B站UP主', '海角老司机', '八卦小报记者', '天涯老涯友']);
            return false;
        }
...
window.MobileContextPlugin = {
    version: '2.4.0',
    description: 'Mobile Context Monitor with Upload & Editor & Custom API & MesID Floor Monitor & Forum Manager & Forum UI v2.4 (SillyTavern.getContext() API Integration)',
