...
class MobileContextEditor {
    ...
    async modifyMessage(messageIndex, newContent, newName = null) {
        try {
            ...
            console.log(`[Mobile Context Editor] 修改消息 ${messageIndex}: "${oldContent.substring(0, 30)}..." → "${newContent.substring(0, 30)}..."`);
    ...
    async deleteMessage(messageIndex) {
        try {
            ...
            this.log('info', `准备删除消息 ${messageIndex}: ${messageToDelete.name}: ${messageToDelete.mes.substring(0, 50)}...`);
    ...
    renderMobileChatMessages() {
        ...
        messages.forEach((message, index) => {
            ...
            html += `
                <div style="margin-bottom: 15px; padding: 12px; border: 2px solid ${isUser ? '#4CAF50' : '#2196F3'}; border-radius: 10px; background: ${isUser ? '#f1f8e9' : '#e3f2fd'};">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                        <strong style="color: #333; font-size: 14px;">${isUser ? '👤' : '🤖'} ${name} (#${index})</strong>
                        <div>
            ...
            const quickEditHtml = `
                <div style="padding: 20px; background: #f8f9fa; border-radius: 8px; margin: 10px 0;">
                    ...
                    <div style="margin-bottom: 15px;">
                        <strong>消息发送者：</strong> ${lastMessage.name || (lastMessage.is_user ? '用户' : '角色')} <br>
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <textarea id="quick-edit-content" style="width: 100%; height: 120px; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-family: inherit; resize: vertical;">${lastMessage.mes}</textarea>
                    </div>

                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 5px; font-weight: bold;">修改发送者名称（可选）：</label>
                        <input type="text" id="quick-edit-name" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;" placeholder="留空保持不变" value="${lastMessage.name || ''}">
                    </div>
    ...
    async testApiConnection() {
        try {
            ...
            if (context) {
                ...
                // 测试4: 角色数据
                addResult('角色数据', Array.isArray(context.characters) ? 'PASS' : 'FAIL', `- ${context.characters?.length || 0} 个角色`);
