...
class ContextMonitor {
            ...
            if (stContext) {
                ...
                context = {
                    ...
                    chat: {
                        ...
                        lastMessage: currentChat.length > 0 ? {
                            id: currentChat.length - 1,
                            name: currentChat[currentChat.length - 1].name || 'Unknown',
                            mes: (currentChat[currentChat.length - 1].mes || '').substring(0, 100) + (currentChat[currentChat.length - 1].mes && currentChat[currentChat.length - 1].mes.length > 100 ? '...' : ''),
                    ...
                    // 角色信息
                    character: (stContext.characterId && stContext.characters[stContext.characterId]) ? {
                        ...
                        description: (stContext.characters[stContext.characterId].description || '').substring(0, 100) + '...'
                    } : null,

                    // 群组信息
                    group: isGroup && stContext.groups ? {
                ...
                context = {
                    ...
                    // 聊天信息
                    chat: {
                        length: (chat && Array.isArray(chat)) ? chat.length : (chat && typeof chat.length === 'number') ? chat.length : 0,
                        lastMessage: (chat && Array.isArray(chat) && chat.length > 0) ? {
                            id: chat.length - 1,
                            name: chat[chat.length - 1].name || 'Unknown',
                            mes: (chat[chat.length - 1].mes || '').substring(0, 100) + (chat[chat.length - 1].mes && chat[chat.length - 1].mes.length > 100 ? '...' : ''),
                    ...
                    // 角色信息
                    character: (this_chid !== undefined && characters && characters[this_chid]) ? {
                        id: this_chid,
    ...
    formatExtractionsAsText(extractions, formatName) {
        ...
        extractions.forEach((extraction, index) => {
            const fieldTexts = format.fields.map(field => {
